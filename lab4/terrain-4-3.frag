...
void main(void)
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        {
            specularStrength = pow(specularStrength, specularExponent[i]); // pow() return the value of the first parameter raised to the power of the second
        }
    ...
    out_Color = vec4(shade, 1.0) * texture(tex, texCoord);;
}